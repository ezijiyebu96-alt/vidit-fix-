...
class ViditApp:
    ...
    def _wire(self) -> None:
        ...
        bus.on("ears.model_downloading", lambda t, p: self.bridge.status.emit("Downloading the speech model (one-time)…"))
        bus.on("ears.model_ready", lambda t, p: self.bridge.status.emit("Speech model ready."))
        bus.on("ears.model_error", lambda t, p: self.bridge.status.emit("Hearing trouble: " + str(p.get("error", ""))[:200]))
        ...
        self.bridge.voiceFinished.connect(self._on_voice_finished)
    ...
    def _on_voice_finished(self) -> None:
        ...
        self._try_autolisten()

    # ------------------------------------------------------------ auto-listen
    def _arm_autolisten(self) -> None:
        """In 'always' mode, don't open the mic until Vidit has finished
        speaking his greeting — otherwise he hears his own first words and
        tries to transcribe them before the speech model is ready."""
        if self._autolisten_armed:
            return
        self._autolisten_armed = True
        QTimer.singleShot(2500, self._try_autolisten)

    def _try_autolisten(self) -> None:
        if not self._autolisten_armed:
            return
        self._autolisten_armed = False
        if self.vidit.config.get("voice.activation", "wake_word") not in ("always",):
            return
        self.vidit.start_listening()
    ...
    def _on_setting(self, key: str, value: Any) -> None:
        ...
        if key.startswith("voice.stt_"):
            # Speech-model settings changed → re-probe/load with the new choices.
            self.vidit.ears.reset_model_cache(clear_downloads=False)
            if self.vidit.ears.status().get("listening"):
                self.vidit.ears.warmup()
    ...
    def run(self) -> int:
        ...
        if self.vidit.config.get("voice.activation") == "always":
            # Wait for his greeting to finish speaking before opening the mic
            # (fallback timer covers silent/no-speech engines).
            self._arm_autolisten()
        code = self.qt.exec_()
        self.vidit.sleep()
        return code
