    ...
    def _toggle_mic(self, on: bool) -> None:
        if on:
            ...
            if not ok:
                ...
                error = self.vidit.ears.status().get("error") or self.vidit.ears.last_error
                self.status_label.setText(error or "microphone not allowed / not available (pip install faster-whisper sounddevice)")
            else:
