...
class ChatWindow(QWidget):
    ...
    def __init__(self, vidit: Vidit, parent: Optional[QWidget] = None):
        ...
        self._typing_timer.timeout.connect(safe_slot(self._animate_typing))
    ...
    def _build(self) -> None:
        ...
        new_btn.clicked.connect(safe_slot(self.new_conversation))
        header.addWidget(new_btn)
        sl.addLayout(header)
        self.folder_box = QComboBox()
        self.folder_box.currentIndexChanged.connect(safe_slot(self._load_conversations))
        sl.addWidget(self.folder_box)
        self.search_box = QLineEdit()
        self.search_box.setPlaceholderText("Search history…")
        self.search_box.returnPressed.connect(safe_slot(self._search_history))
        sl.addWidget(self.search_box)
        self.conv_list = QListWidget()
        self.conv_list.itemClicked.connect(safe_slot(self._open_conversation))
        self.conv_list.setContextMenuPolicy(Qt.CustomContextMenu)
        self.conv_list.customContextMenuRequested.connect(safe_slot(self._conv_menu))
        sl.addWidget(self.conv_list, 1)
        pins_btn = QPushButton("📌 Pinned")
        pins_btn.clicked.connect(safe_slot(self._show_pinned))
        ...
        self.think_toggle.toggled.connect(safe_slot(lambda on: setattr(self.vidit, "show_thinking", on)))
        top.addWidget(self.think_toggle)
        export_btn = QPushButton("Export")
        export_btn.clicked.connect(safe_slot(self._export_menu))
        ...
        self.view.customContextMenuRequested.connect(safe_slot(self._message_menu))
        ...
        attach_btn.clicked.connect(safe_slot(self._pick_files))
        ...
        self.input.submitted.connect(safe_slot(self.send))
        bottom.addWidget(self.input, 1)
        self.send_btn = QPushButton("Send")
        self.send_btn.setObjectName("primary")
        self.send_btn.clicked.connect(safe_slot(self.send))
        bottom.addWidget(self.send_btn)
        stop_btn = QPushButton("■ Stop")
        stop_btn.setToolTip("Stop (he stops immediately)")
        stop_btn.clicked.connect(safe_slot(self._stop))
